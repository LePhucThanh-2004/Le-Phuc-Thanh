...
// Hàm để tìm nước đi tốt nhất cho máy tính
function findBestMove() {
  let bestMove = null;
  let bestScore = -Infinity;
  for (let i = 0; i < 8; i++) {
      for (let j = 0; j < 8; j++) {
          if (pieces[i * 8 + j] !== '' && pieces[i * 8 + j].charCodeAt(0) >= 9812 && pieces[i * 8 + j].charCodeAt(0) <= 9817) {
              // Thử mỗi nước đi có thể của quân cờ hiện tại
              for (let k = 0; k < 8; k++) {
                  for (let l = 0; l < 8; l++) {
                      if (isValidMove(i, j, k, l)) {
                          // Tạm thời thực hiện nước đi
                          const tempPiece = pieces[k * 8 + l];
                          pieces[k * 8 + l] = pieces[i * 8 + j];
                          pieces[i * 8 + j] = '';
                          // Đánh giá điểm cho nước đi này
                          const score = minimax(3, -Infinity, Infinity, false);
                          // Hoàn trả lại trạng thái ban đầu của bảng
                          pieces[i * 8 + j] = pieces[k * 8 + l];
                          pieces[k * 8 + l] = tempPiece;
                          // Lưu nước đi tốt nhất cho máy tính
                          if (score > bestScore) {
                              bestScore = score;
                              bestMove = { fromX: i, fromY: j, toX: k, toY: l };
                          }
                      }
                  }
              }
...
// Hàm đánh giá điểm cho mỗi nước đi
function minimax(depth, alpha, beta, isMaximizingPlayer) {
  // Kiểm tra nếu đã đạt đến độ sâu tối đa hoặc trò chơi kết thúc
  if (depth === 0) {
  ...
  if (isMaximizingPlayer) {
      ...
      for (let i = 0; i < 8; i++) {
          for (let j = 0; j < 8; j++) {
              if (isValidMove() /* Hàm kiểm tra nước đi hợp lệ */) {
                  // Thực hiện nước đi
                  // Đánh giá nước đi
                  // Gọi lại minimax cho nước đi tiếp theo
                  // Cập nhật maxScore
              }
          }
      }
...
function evaluateBoard() {
  // Thực hiện đánh giá các yếu tố của bảng cờ (ví dụ: số lượng quân cờ, vị trí của các quân cờ, ...)
  // Trả về một điểm số đại diện cho tình hình trên bảng
}

// Hàm kiểm tra tính hợp lệ của một nước đi
function isValidMove(/* Tham số của nước đi */) {
